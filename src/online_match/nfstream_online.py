...
def streamer(offline_file, log_file):
    streamer = NFStreamer(source='Intel(R) Ethernet Connection (2) I219-LM',
                          udps=FlowFilter(offline_file=offline_file, log_file=log_file))
    # streamer = NFStreamer(source='E:/project/Shrink/data/record/test/pcap/11_2023_09_26_20_39_32.pcap',
    #                       udps=FlowFilter(offline_file=offline_file, log_file=log_file))
    return streamer
...
if __name__ == '__main__':
    ...
    online_file = 'E:/project/Shrink/data/fingerprint/nfstream_online.csv'
    t = time.strftime('%m_%d_%H_%M', time.localtime(time.time()))
    log_file = 'E:/project/Shrink/data/result/log_' + t + '.txt'
    f = open(log_file, 'a')
    f.close()
