...
# 在线模式马尔可夫
class Markov_alg():
    def __init__(self, online_file, offline_file, offline_audio_thd, high_orders, high_bins_count, high_win_size,
...
if __name__ == '__main__':
    online_file = '../../data/fingerprint/traffic_fingerprint.csv'
    offline_file = '../../data/fingerprint/video_fingerprint.csv'
    offline_audio_thd = 600 * 1024
    result = []
    for bucketnum in range(1, 201):
        print(bucketnum)
        result_bucket = [bucketnum]
        for i in range(2, 30):
            for j in range(1, 30 - i):
                high_orders, high_bins_count, high_win_size = i, bucketnum, j
                low_orders, low_bins_count, low_win_size = 1, bucketnum, i + j - 1
    ...
    with open('../../data/result/result_' + t + '.data', 'wb') as f:
