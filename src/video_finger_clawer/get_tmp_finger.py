...
class Finger_valid_val():
    def __init__(self, loss_first_chunk_flag, break_count, stream_count, mix_count, chunk_count, seq_video_itag_val,
                 seq_video_itag_tuple_val) -> None:
...
class Finger():
        ...
        for path, finger_valid_dict in finger_valid_dict_list.items():
            for itag in finger_valid_dict:
                ...
                if finger_valid_dict[itag].chunk_count > 5:
                    ...
                    # 输出累计信息
                    recoed_file.write("," + str(finger_valid_dict[itag].loss_first_chunk_flag) + "," + str(
                        finger_valid_dict[itag].break_count) + "," + str(
                        finger_valid_dict[itag].stream_count) + "," + str(
                        finger_valid_dict[itag].mix_count) + "," + str(finger_valid_dict[itag].chunk_count) + ",")
                    # 输出连续块列表
    ...
    def finger_valid(self, video_itag_dict):
        ...
        for itag in video_itag_dict:
            ...
            cons_video_itag_val.append(cons_chunk_count)
            cons_video_itag_tuple_val.append(pre_tuple)
            finger_valid_val = Finger_valid_val(miss_beg_flag, chunk_miss_flag, len(tuple_dict), mix_count,
                                                len((video_itag_dict[itag])), cons_video_itag_val,
                                                cons_video_itag_tuple_val)
