    ...
    def video_fingerprint_down(self, number, video_name):
        ...
        for cur_resolution in target_resolution:
            ...
            for i in response_headers:
                if 'videoplayback' in i['name']:
                    mitm_label.append(i['name'])
            mitm_file = open(new_mitm_path + time_name, mode='a+', encoding='utf-8')
            for i in mitm_label:
                mitm_file.write('-1\n')
                mitm_file.write(i + '\n')
                mitm_file.write('*\n')
