    ...
    for finger_data in finger_datas:
        ...
        for index_data in index_datas:
            if index_data.find(finger_data) != -1:
                video_name.append(
                    index_data[(int(index_data.find('----')) + 4):(int(index_data.find(finger_data)) - 4)])
...
def get_finger_main():
    ...
    for file_path, filename in zip(Filelist, filenames):
        print(filename[:-4])
        if filename[:-4] == "index" or filename[-4:] != ".txt" or filename[:-4] == "fingerpint2" or filename[
                                                                                                    :-4] == "fingerpint" or filename[
                                                                                                                            :-4] == "fingerpint_audio":
...
def from_index_get_pcapname(index_path, record_path, type):
    ...
    for index_data in index_datas:
        ...
        if beg != -1:
            record_file.write(
                './sapp -d -r ../video_title_classification/data/tencent/{}/{} --dumpfile-speed top-speed\n'.format(
                    type, index_data[beg + 4:end + 4]))
