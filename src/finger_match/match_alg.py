...
class Match_alg():
    ...
    def P_dtw(self):
        ...
        for i in range(len(self.offline_chunk_list)):
            ...
            for j in range(1, len(self.offline_chunk_list[i].finger_list)):
                r = (self.offline_chunk_list[i].finger_list[j] - self.offline_chunk_list[i].finger_list[j - 1]) / \
                    self.offline_chunk_list[i].finger_list[j - 1]
                R.append(r)
            if R != [0]:
    ...
    def markov_hight_order(self, bins_count, orders, win_size, de_mix_stream_flag=0, mutil_order_flag=0, bias=3000,
                           error_bins_tuples_dict={}):
        time_sum = 0
        time_count = 0
        orders += 1
        # 计算离线指纹的order阶概率转移矩阵
...
if __name__ == '__main__':
    ...
    for i in range(1, 6, 1):
        for j in range(4000, 7000, 1000):
            ...
            all_count, true_count, acc = match_alg.pred_performance()
            print('{},{},{},{},{},{},{},{}'.format(i, j, error_count, online_short_count, all_count, true_count, acc,
                                                   true_count / (error_count + all_count)))
    ...
    for i in bin_count:
        for j in range(1, 11, 1):
            for k in range(1, 10, 1):
                ...
                all_count, true_count, acc = match_alg.pred_performance()
                print(
                    '{},{},{},{},{},{},{},{},{}'.format(i, j, k, error_count, online_short_count, all_count, true_count,
                                                        acc, true_count / (error_count + all_count)))
